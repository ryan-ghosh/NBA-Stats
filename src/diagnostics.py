...
diagnostics.py
==============
Convergence diagnostics for the Gibbs chains: trace plots, autocorrelation,
effective sample size, and the Gelman–Rubin split-chain R-hat.

Implementations follow Vehtari et al. (2021) "Rank-normalization, folding,
and localization: An improved R-hat", but we report the simpler split-chain
R-hat (no rank normalization) which is what Hoff Ch. 6 introduces.
"""
...
def split_chains(x: np.ndarray) -> np.ndarray:
    """Split each chain in half along the iteration axis. x is (M, T, ...)."""
    M, T = x.shape[0], x.shape[1]
    half = T // 2
    a = x[:, :half]
    b = x[:, half:2 * half]
    return np.concatenate([a, b], axis=0)   # (2M, T/2, ...)


def rhat(x: np.ndarray) -> float | np.ndarray:
    """Split-chain potential scale reduction R-hat. x: (M, T) or (M, T, K)."""
    s = split_chains(x)                  # (2M, T/2, ...) or (2M, T/2)
    M, T = s.shape[0], s.shape[1]
    chain_mean = s.mean(axis=1)          # (2M, ...)
    grand_mean = chain_mean.mean(axis=0) # (...)
    B = T * np.var(chain_mean, axis=0, ddof=1)   # between-chain variance
    W = np.mean(np.var(s, axis=1, ddof=1), axis=0)  # within-chain variance
    var_hat = (1 - 1 / T) * W + B / T
    return np.sqrt(var_hat / W)


def ess(x: np.ndarray, max_lag: int | None = None) -> float | np.ndarray:
    """Bulk effective sample size via integrated autocorrelation, computed
    by combining all chains. x: (M, T) or (M, T, K). Uses Geyer's initial
...
def _ess_1d(z: np.ndarray, max_lag: int | None = None) -> float:
    ...
    if var == 0:
        return float(n)
    # FFT autocorrelation for speed
    f = np.fft.fft(z, n=2 * n)
    acf = np.fft.ifft(f * np.conj(f))[:n].real
    acf = acf / acf[0]
    # Geyer initial monotone sequence: sum pairs (rho_{2k} + rho_{2k+1})
    # while positive and monotonically decreasing.
    rho_pair = acf[:-1:2] + acf[1::2]   # length n//2
    # Truncate at first non-positive pair
    cum_min = np.minimum.accumulate(rho_pair)
    # Stop at first pair where cumulative min becomes <= 0
    keep = cum_min > 0
    if not keep.any():
        return float(n)
...
def trace_plot(samples: dict, params: list[tuple[str, str]],
               out_path: Path) -> None:
    """params: list of (key, label). samples[key] shape (M, T) or (M, T, K) (we slice)."""
    n = len(params)
    ...
    for i, (k, label) in enumerate(params):
        ...
        if i == n - 1: ax.set_xlabel("Lag", fontsize=9)
        if i == 0: ax.set_title("Autocorrelation (combined chains)", fontsize=10)
    fig.tight_layout()
...
def summarize(npz_path: Path, out_dir: Path, label: str) -> dict:
    ...
    # Save markdown table for the writeup
    ...
    # One sample team and season too
